...
def main_post():
    if request.method == 'POST':
        ...
        if not alpha.isdecimal():
            return render_template('index.html', developer_name = 'Sinem sipahi', not_valid = True)
        if not (0 < int(alpha)):
            return render_template('index.html', developer_name = 'Sinem sipahi', not_valid = True)
        return render_template('result.html', developer_name=' Sinem Sipahi', milliseconds = alpha, result = convert(int(alpha)))
    else:
        return render_template('index.html', developer_name ='Sinem sipahi', not_valid = False)
